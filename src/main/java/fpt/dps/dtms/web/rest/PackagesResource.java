...
public class PackagesResource {
    ...
    public PackagesResource(PackagesService packagesService, PackagesQueryService packagesQueryService, TasksService tasksService) {
    ...
    /**
     * POST  /packages : Create a new packages and clone tasks
     *
     * @author PhuVD3
     * @throws Exception 
     */
    @PostMapping("/packages-clone")
    @Timed
    @PreAuthorize("@jhiAuth.hasPermission('PACKAGE', 'CREATE')")
    public ResponseEntity<PackagesDTO> clonePackages(@Valid @RequestBody PackagesDTO packagesDTO) throws Exception {
        log.debug("REST request to save Packages : {}", packagesDTO);
        Long packagesId = packagesDTO.getId();
        List<TasksDTO> lstTasks = tasksService.findListTasksClone(packagesId);
        packagesDTO.setId(null);
        ...
        /*Move Tasks*/
        tasksService.moveTasksClone(lstTasks, result.getId());
     ...
     * @return
     */
    @PutMapping("/packages-all/{packageId}/{purchaseOrderId}/{packagesLocal}")
    @Timed
    @PreAuthorize("@jhiAuth.hasPermission('PACKAGE', 'CREATE')")
    public ResponseEntity<Void> cloneAllTasks(@PathVariable Long packageId, @PathVariable Long purchaseOrderId, @PathVariable Long packagesLocal) {
        log.debug("REST request to Clone Packages : {}", packageId);
        
        /*find task have All status*/
        List<TasksDTO> lstTasks = tasksService.findAllListTasksClone(packagesLocal);
        tasksService.cloneTasks(lstTasks, packageId);
        try {
        	return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, packageId.toString())).build();
        } catch(Exception e) {
        	return ResponseEntity.badRequest().headers(HeaderUtil.serviceFailureAlert(ENTITY_NAME, AppConstants.IN_USED, "A package cannot clone")).build();
        }
    }

    /**
     * clone All Tasks have status OPEN
     * @param packageId
     * @param purchaseOrderId
     * @return
     */
    @PutMapping("/packages-open/{packageId}/{purchaseOrderId}/{packagesLocal}")
    @Timed
    @PreAuthorize("@jhiAuth.hasPermission('PACKAGE', 'CREATE')")
    public ResponseEntity<Void> cloneOpenTasks(@PathVariable Long packageId, @PathVariable Long purchaseOrderId, @PathVariable Long packagesLocal) {
        log.debug("REST request to Clone Packages : {}", packageId);
        List<TasksDTO> lstTasks = new ArrayList<TasksDTO>();
        /*find task have status OPEN*/
        lstTasks = tasksService.findListTasksClone(packagesLocal);
        tasksService.cloneTasks(lstTasks, packageId);
        try {
        	return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, packageId.toString())).build();
        } catch(Exception e) {
