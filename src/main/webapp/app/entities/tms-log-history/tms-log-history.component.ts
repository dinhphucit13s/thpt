...
@Component({
    selector: 'jhi-tms-log-history',
    templateUrl: './tms-log-history.component.html',
    providers: [PermissionService]
})
export class TMSLogHistoryComponent implements OnInit, OnDestroy {

currentAccount: any;
    tMSLogHistories: TMSLogHistory[];
    error: any;
    success: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.tMSLogHistoryService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<TMSLogHistory[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.tMSLogHistoryService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<TMSLogHistory[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/tms-log-history'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/tms-log-history', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    ngOnInit() {
        this.permissionService.findPermission().subscribe((roleResponse: HttpResponse<Permission>) => {
            ...
            for (let resource of this.allPermission.resources) {
                ...
                if (resource.name == 'TMS_LOG_HISTORY') {
    ...
    trackId(index: number, item: TMSLogHistory) {
        return item.id;
    }
    ...
    registerChangeInTMSLogHistories() {
        this.eventSubscriber = this.eventManager.subscribe('tMSLogHistoryListModification', (response) => this.loadAll());
    }
