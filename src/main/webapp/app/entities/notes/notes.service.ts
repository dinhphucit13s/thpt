...
export class NotesService {
    ...
    search(req?: any): Observable<HttpResponse<Notes[]>> {
        const options = createRequestOption(req);
        return this.http.get<Notes[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Notes[]>) => this.convertArrayResponse(res));
    }

    getAttachmentByNotesId(notesId: any): Observable<HttpResponse<any[]>> {
        console.log(notesId);
        return this.http.get<any[]>(`${this.resourceUrl}/attachment?notesId=${notesId}`, { observe: 'response'});
    }

    createNotesOfBugs(formData: any): Observable<HttpResponse<Notes>> {
        return this.http.post<Notes>(this.resourceUrl + '/createNotesOfBug', formData, { observe: 'response' });
    }
