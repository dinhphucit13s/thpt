...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.notificationTemplateService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<NotificationTemplate[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.notificationTemplateService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<NotificationTemplate[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/notification-template'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: NotificationTemplate) {
        return item.id;
    }
    registerChangeInNotificationTemplates() {
        this.eventSubscriber = this.eventManager.subscribe('notificationTemplateListModification', (response) => this.loadAll());
    }
